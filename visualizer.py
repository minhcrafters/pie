...
import numpy as np
from pie.audio import AudioSource
from pie.engine import Engine
from pie.entity import Camera, Entity
from pie.light import Light
from pie.mesh import Mesh
from pie.texture import Texture
...
def main():
    engine = Engine("pie", WINDOW_W, WINDOW_H)
    ...
    while running:
        ...
        if started:
            ...
            for i, e in enumerate(bars):
                ...
                # dynamic color interpolation from green (low) to red (high)
                vv = clamp(v, 0.0, 1.0)
                # base gradient from green (vv=0) to red (vv=1)
                base_r = vv
                base_g = 1.0 - vv
                base_b = 0.0
                # blend with a light (almost white) color to create pastel tones
                pastel_strength = 0.1
            ...
            # modulate directional light intensity/color slightly with audio energy for responsiveness
            overall_energy = float(smoothed.mean()) if smoothed.size else 0.0
            # base white directional light, modulate brightness between 0.2 and 1.0
            brightness = clamp(0.3 + overall_energy * 0.9 + bass_norm * 0.4, 0.05, 1.5)
            directional_light.color = (brightness, brightness, brightness)

            # OSC smoothing & positions (unchanged)
            ...
            for i, e in enumerate(osc_spheres):
                ...
                e.position = (x, y, OSC_Z)

            # basic input handling (camera capture/movement)
